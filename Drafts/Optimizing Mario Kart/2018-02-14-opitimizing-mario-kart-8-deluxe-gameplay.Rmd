...
noms <- c("Mario","Luigi","Peach","Daisy","Rosalina","Tanooki Mario","Cat Peach",paste("Yoshi",1:9, sep="_"),"Toad","Koopa Troopa",paste("Shy Guy",1:9, sep="_"),"Lakitu","Toadette","King Boo","Baby Mario", "Baby Luigi", "Baby Peach", "Baby Rosalina","Metal Mario", "Gold Mario","Pink Gold Peach","Wario","Waluigi","Donkey Kong","Bowser","Dry Bones","Bowser Jr.","Dry Bowser","Larry","Wendy","Ludwig","Iggy","Roy","Morton",paste("Inkling Girl",1:3,sep="_"),paste("Inkling Boy",1:3,sep="_"),"Link","Male Villager","Female Villager","Isabelle")

  
x <-c((0:24),(0:24),(0:9))
y <- c(rep(752,24), rep(820,24), rep(882,9))

test <- function(x,y,name) {
  frame <- paste("60x60+",x*65+2,"+",y, sep="")
  id <- paste("img",y,x,sep="_")
  assign(x = id ,value = image_crop(MarioK_img, frame)) %>%
    image_write(.,path = paste("icons/img",y,j,".png", sep="_"), format="png")
  
    m <- readPNG(paste("icons/img",y,x,".png",sep="_"), F)
    m[,,4] <- m[,,4]*0.6 # adjust alpha
    writePNG(m, paste(paste("icons/",name,".png", sep="")))
    }
...
```{r}
test <- function(names){
for (j in c(rep(752,24), rep(820,24), rep(882,12))) {
  for(i in i+1) {
  frame <- paste("60x60+",i*65+2,"+",j, sep="")
  name <- paste("img",i,j,sep="_")
  assign(x = name,value = image_crop(MarioK_img, frame)) %>%
  image_write(.,path = paste("icons/img",i,j,".png", sep="_"), format="png")
  
    m <- readPNG(paste("icons/img",i,j,".png",sep="_"), F)
    m[,,4] <- m[,,4]*0.6 # adjust alpha
  }
writePNG(m, paste("icons/",noms,".png", sep=""))
