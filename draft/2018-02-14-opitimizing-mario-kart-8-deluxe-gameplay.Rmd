...
Video games are about learning to make good decisions. I always ejoyed the liberty they offer through the choices available. And this is why I think Zelda : Breath Of the Wild definitly changed something in the open world circle of games. but that's not the subject today (probably another time though).

Mario Kart was one of my first game I played as a kid. Then on SNES and now on Switch. Back then the was was simple between big and heavy donkey kong or the sneaky Toad, now we get to chose the character, the kart but also its tires and glider (yes, mario kart is also about glidding now).
...
In its annotations, we can see that there are many redundancies in the karts and parts : 
...
So refer to this table if you want equivalent stats but different skins.

Instead of directly importing data from google drive, I built an excel file 
...
ggplot(data_combo %>% mutate(image=case_when(Char=="Mario" ~ "P:/R/Github/source/draft/img_16_752_.png")), aes(Speed_mean, Accel,alpha=`M-turbo`))+
...
Plot with images
```{r}
MarioK_img <- image_read("P:/R/Github/source/draft/Mario Kart.png")
print(MarioK_img)
image_info(MarioK_img)

for (i in c((0:24),(0:24),(0:11))) {
  for(j in c(rep(752,24), rep(820,24), rep(882, 11))){
  frame <- paste("60x60+",i*65+2,"+",j, sep="")
  name <- paste("img",i,j,sep="_")
  assign(x = name,value = image_crop(MarioK_img, frame)) %>%image_write(.,path = paste("img",i,j,".png", sep="_"), format="png")
  
    m <- readPNG(paste("P:/R/Github/source/draft/img",i,j,".png",sep="_"), F)
    w <- matrix(rgb(m[,,1],m[,,2],m[,,3],m[,,4] * 0.4), nrow=dim(m)[1]) # adjust alpha
...
for (i in c((0:24),(0:24),(0:11))) {
  ...
  assign(x = name,value = image_crop(MarioK_img, frame)) %>%image_write(.,path = paste("img",i,j,".png", sep="_"), format="png")
    ...
    m <- readPNG(paste("P:/R/Github/source/draft/img",i,j,".png",sep="_"), F)
    w <- m
    w[,,4] <- m[,,4]*0.7 # adjust alpha
    ggsave(writePNG(w, paste(paste("img",i,j,".png", sep="_"))), device = "png")
  }
}
